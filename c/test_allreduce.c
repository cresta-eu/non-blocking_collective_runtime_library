...
int main(int argc, char *argv[])
{
   ...
   if (my_id==0) {
      sprintf(filename, "cresta_allreduce%i.dat", ntask);
      fp=fopen(filename,"a");
...
/* benchmark loop over message sizes */
   ...
   for(imsg=0; imsg<=16; imsg+=2){ 
     ...
     recvbuf = (int*) malloc(n*sizeof(int)); 
     testsendbuf = (int*) malloc(n*sizeof(int));
     testrecvbuf = (int*) malloc(n*sizeof(int));
     if (n <= large_msg) {
       maxiter = 50;
     } else {
       maxiter = 10;
     ...
     for (iter=1; iter<=maxiter; iter++) {
        t0 = MPI_Wtime();
        CRESTA_Allreduce(sendbuf, recvbuf, n, MPI_INT, MPI_SUM, 
                      MPI_COMM_WORLD); 
        t1 = MPI_Wtime() - t0;
        time += t1;
     } 
     ...
     MPI_Allreduce(testsendbuf, testrecvbuf, n, MPI_INT, MPI_SUM,
                    MPI_COMM_WORLD);
     for (i=0; i<n; i++) {
        if (recvbuf[i] != testrecvbuf[i]) {
           printf("Error in CRESTA_Reduce, n=%i, my_id=%i, %i %i \n",
