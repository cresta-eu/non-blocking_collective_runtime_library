...
int main(int argc, char *argv[])
{
   ...
   if (my_id==0) {
      sprintf(filename, "cresta_gather%i.dat", ntask);
      fp=fopen(filename,"a");
...
/* benchmark loop over message sizes */
   ...
   for(imsg=0; imsg<=16; imsg+=2){ 
     ...
     for (iter=1; iter<=maxiter; iter++) {
        t0 = MPI_Wtime();
        CRESTA_Gather(sendbuf, n, MPI_INT, recvbuf, n, MPI_INT, 
                      root, MPI_COMM_WORLD); 
        t1 = MPI_Wtime() - t0;
        time += t1;
     } 
     ...
     MPI_Gather(testsendbuf, n, MPI_INT, testrecvbuf, n, MPI_INT,
                root, MPI_COMM_WORLD);
     if (my_id==root){
       for (i=0; i<n*ntask; i++) {
          if (recvbuf[i] != testrecvbuf[i]) {
             printf("Error in CRESTA_Gather, n=%i, my_id=%i, %i %i \n",
