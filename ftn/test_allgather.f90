program collectives_benchmark
  ...
  write(filename,'(A16,I0,A4)') 'cresta_allgather', ntasks, '.dat'
  if(my_id==0) open(12,file=filename,form='formatted')

  do imsg = 1,14,2
     n = 2**imsg
     allocate (cresta_send(n), cresta_recv(n*ntasks))
     allocate (test_send(n), test_recv(n*ntasks))
     ...
     do iter = 0, maxiter
       t0 = gettime()      
       call CRESTA_Allgather(cresta_send, n, mpi_integer, &
...
! compare against MPI
     call MPI_Allgather(test_send, n, mpi_integer, &
		     test_recv, n, mpi_integer, &
                     mpi_comm_world, rc)
     if (any(cresta_recv(:) /= test_recv(:))) then
        write(*,*) ' Error in CRESTA_Allgather'
        do i = 1, n
