...
!  use os_collectives
  ...
  integer :: n, imsg, i, cnt, iter, maxiter, root
...
!  call cresta_win_init(mpi_comm_world, rc)
  ...
  write(filename,'(A14,I0,A4)') 'cresta_reduce', ntasks, '.dat'
  ...
  do imsg = 0,24,4
     ...
     do iter = 0, maxiter
       t0 = gettime()      
       call CRESTA_Reduce(cresta_send, cresta_recv, n, &
...
! compare against MPI
     call MPI_Reduce(test_send, test_recv, n*ntasks, &
                            mpi_integer, MPI_SUM, root, &
                            mpi_comm_world, rc)

     if (my_id == root) then
       if (any(cresta_recv(:) /= test_recv(:))) then
          write(*,*) ' Error in CRESTA_Reduce'
          do i = 1, n
