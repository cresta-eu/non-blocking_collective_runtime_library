...
!  call cresta_win_init(mpi_comm_world, rc)
  ...
  write(filename,'(A14,I0,A4)') 'cresta_scatter', ntasks, '.dat'
  if(my_id==0) open(12,file=filename,form='formatted')

  do imsg = 1,14,2
     n = 2**imsg
     allocate (cresta_send(n*ntasks), cresta_recv(n))
     allocate (test_send(n*ntasks), test_recv(n))
     ...
     do iter = 0, maxiter
       t0 = gettime()      
       call CRESTA_Scatter(cresta_send, n, mpi_integer, &
...
! compare against MPI
     call MPI_Scatter(test_send, n, mpi_integer, &
		     test_recv, n, mpi_integer, &
                     root, mpi_comm_world, rc)
     if (any(cresta_recv(:) /= test_recv(:))) then
        write(*,*) ' Error in CRESTA_Scatter'
        do i = 1, n
